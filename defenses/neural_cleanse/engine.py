...
def train_trigger_neural_cleanse(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer, lr_scheduler, device: torch.device, 
                    class_mask=None, args=None, regression_model=None,):
    # create matrix to save end-of-task accuracies 
    acc_matrix = np.zeros((args.num_tasks, args.num_tasks))
    feature, feature_mat = None, None
    key_feature, key_feature_mat = None, None

    recorder = utils.Recorder(args)
    task_id = 0
    for epoch in range(args.gen_round):
        train_stats, early_stop = train_one_epoch_trigger_neural_cleanse(model=model, original_model=original_model, criterion=criterion,
                                    data_loader=data_loader[task_id]['val'], optimizer=optimizer, device=device,
                                    epoch=epoch, feature_mat=feature_mat, key_feature_mat=key_feature_mat, max_norm=args.clip_grad,
                                    set_training_mode=False, task_id=task_id, class_mask=class_mask, args=args, regression_model=regression_model, recorder=recorder)
...
def train_one_epoch_trigger_neural_cleanse(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, feature_mat, key_feature_mat, max_norm: float = 0,
                    set_training_mode=True, task_id=-1, class_mask=None, args = None, regression_model=None, recorder=None, train=True):
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        new_images = torch.clone(input)
        new_images = regression_model(new_images)

        target = target_labels.to(device, non_blocking=True)
        ...
        loss_ce = criterion(logits, target) # base criterion (CrossEntropyLoss)
        loss_reg = torch.norm(regression_model.get_raw_mask(), 1)
        loss = loss_ce + recorder.cost * loss_reg
        # if args.pull_constraint and 'reduce_sim' in output:
        #     loss = loss - args.pull_constraint_coeff * output['reduce_sim']
        if loss.item() < 0:
            print(logits, target)
        ...
        minibatch_accuracy = torch.sum(torch.argmax(logits, dim=1) == target_labels).detach() * 100.0 / sample_num
        loss_ce_list.append(loss_ce.detach())
        loss_reg_list.append(loss_reg.detach())
        loss_list.append(loss.detach())
        loss_acc_list.append(minibatch_accuracy)

        true_pred += torch.sum(torch.argmax(logits, dim=1) == target_labels).detach()
    ...
    if avg_loss_acc >= 98.0 and avg_loss_reg < recorder.reg_best:
    ...
    if not inner_early_stop_flag:
        ...
        if recorder.cost == 0 and avg_loss_acc >= 98.0:
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, inner_early_stop_flag
