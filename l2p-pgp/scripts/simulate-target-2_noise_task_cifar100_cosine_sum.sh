...
OUTDIR=logs_simulate/attack_target_${lab}
...
do
for tri_epochs in 50 20 #20 50 #10 20
do
for tri_reg_coef in 1e-2 1e-3 1e-5 #1e-4 1e-3 1e-1 1e-5
do
# process inputs
SUBOUTDIR=logs_push_cosine_sum/cifar100/attack_target_${lab}/output_simulate_noise_task/tri_lr_${tri_lr}_p_lr_${p_lr}_tri_epochs_${tri_epochs}/tri_reg_coef_${tri_reg_coef}
mkdir -p ./${SUBOUTDIR}
...
python main_victim.py 10cifar100_l2p_pgp --model vit_base_patch16_224 --epochs 5 --output_dir .${SUBOUTDIR}/output_victim \
        --num_tasks 10 --no_pgp --target_lab $lab --noise_path ${SUBOUTDIR}/checkpoint/best_noise_tuned.npy \
