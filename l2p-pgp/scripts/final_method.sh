...
#         --num_tasks 5 --no_pgp --target_lab $lab --noise_path ${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef

python main_victim.py cub200_l2p_pgp --model vit_base_patch16_224 --epochs 5 --output_dir .${SUBOUTDIR}/output_victim_pgp \
        --num_tasks 5 --target_lab $lab --noise_path ${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef
done
done
done
done