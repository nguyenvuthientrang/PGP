...
do
for load_balancing_coef in 1e-1 1e-2 1e-3 1e-4
do
# process inputs
SUBOUTDIR=logs_load_balancing_task2/cub200/attack_target_${lab}/output_simulate_noise_task/tri_lr_${tri_lr}_p_lr_${p_lr}_tri_epochs_${tri_epochs}/load_balancing_coef_${load_balancing_coef}
mkdir -p ./${SUBOUTDIR}
...
python main_simulate.py cub200_l2p_pgp --model vit_base_patch16_224 --output_dir ./${SUBOUTDIR} \
        ...
        --simulate_lr_prompt $p_lr --simulate_lr_tri $tri_lr --load_balancing --load_balancing_coef $load_balancing_coef 
