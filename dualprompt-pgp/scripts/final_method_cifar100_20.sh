...
# python main_simulate.py cifar100_l2p_pgp --model vit_base_patch16_224 --output_dir ./${SUBOUTDIR} \
...
VICOUTDIR=logs_bce_cosine_sum/cifar100_20/attack_target_${lab}/output_simulate_noise_task/tri_lr_${tri_lr}_p_lr_${p_lr}_tri_epochs_${tri_epochs}/tri_reg_coef_${tri_reg_coef}
mkdir -p ./${VICOUTDIR}/output_victim
...
python main_victim.py 20cifar100_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${VICOUTDIR}/output_victim \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 20 --no_pgp --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef

mkdir -p ./${VICOUTDIR}/output_victim_pgp

python main_victim.py 20cifar100_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${VICOUTDIR}/output_victim_pgp \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 20 --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef
done
done
done
done