...
python main_victim.py 10cifar100_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${SUBOUTDIR}/output_victim \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 10 --no_pgp --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef

mkdir -p ./${SUBOUTDIR}/output_victim_pgp

python main_victim.py 10cifar100_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${SUBOUTDIR}/output_victim_pgp \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 10 --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef
done
done
done
done