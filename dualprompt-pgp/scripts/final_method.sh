...
do
# process inputs
SUBOUTDIR=logs_bce_cosine_sum/cub200/attack_target_${lab}/output_simulate_noise_task/tri_lr_${tri_lr}_p_lr_${p_lr}_tri_epochs_${tri_epochs}/tri_reg_coef_${tri_reg_coef}
# mkdir -p ./${SUBOUTDIR}
...
python main_victim.py cub200_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${SUBOUTDIR}/output_victim \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 5 --no_pgp --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef

mkdir -p ./${SUBOUTDIR}/output_victim_pgp

python main_victim.py cub200_dualprompt_pgp --model vit_base_patch16_224 --output_dir .${SUBOUTDIR}/output_victim_pgp \
        --data-path /home/ubuntu/neurips2024/PGP/l2p-pgp/data \
        --num_tasks 5 --target_lab $lab --noise_path /home/ubuntu/neurips2024/PGP/l2p-pgp/${SUBOUTDIR}/checkpoint/best_noise_tuned.npy  --use_bce --push --tri_reg_distance cosine --tri_reg_reduction sum --tri_reg_coef $tri_reg_coef
done
done
done
done